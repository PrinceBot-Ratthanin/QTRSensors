...
int QTRSensors::readline(bool colorLine){
  bool onLine = false;
  long avg = 0;
  long sum = 0;
  for (uint8_t i = 0; i < _sensorCount; i++) 
  {
    ...
    long value = map(analogRead(_sensorPins[i]), calibrationOn.minimum[i], calibrationOn.maximum[i], 100, 0);
    if (colorLine == 1) { value = 100 - value; }                                                                            
    if (value > 25) {
      onLine = true;
    }
    if (value > 5)
  ...
  if (!onLine)
  {
    if (_lastPosition < (_sensorCount - 1) * 100 / 2)
    {
      return  0 - ((_sensorCount - 1) * 100)/2;
    }
    else 
    {
      return ((_sensorCount - 1) * 100) - ((_sensorCount - 1) * 100)/2;
    }
  }
  _lastPosition = avg / sum;
  return _lastPosition - ((_sensorCount - 1) * 100)/2; 

}
void QTRSensors::setSensorMin(uint16_t value_S1,uint16_t value_S2,uint16_t value_S3,uint16_t value_S4,uint16_t value_S5,uint16_t value_S6,uint16_t value_S7,uint16_t value_S8,uint8_t num_Sensor)
{
  uint16_t _sensorValues[] = {value_S1,value_S2,value_S3,value_S4,value_S5,value_S6,value_S7,value_S8};
  for (uint8_t i = 0; i < num_Sensor; i++)
  {
    _min_sensor_values[i] = _sensorValues[i];
  }
}
void QTRSensors::setSensorMax(uint16_t value_S1,uint16_t value_S2,uint16_t value_S3,uint16_t value_S4,uint16_t value_S5,uint16_t value_S6,uint16_t value_S7,uint16_t value_S8,uint8_t num_Sensor)
{
  uint16_t _sensorValues[] = {value_S1,value_S2,value_S3,value_S4,value_S5,value_S6,value_S7,value_S8};
  for (uint8_t i = 0; i < num_Sensor; i++)
  {
    _max_sensor_values[i] = _sensorValues[i];
  }
}
int QTRSensors::readline_sensor(bool colorLine,uint8_t numOfSensor,int value_S1,int value_S2,int value_S3,int value_S4,int value_S5,int value_S6,int value_S7,int value_S8){
  bool onLine = false;
  long avg = 0;
  long sum = 0;
  int _sensorValues[] = {value_S1,value_S2,value_S3,value_S4,value_S5,value_S6,value_S7,value_S8};
  for (uint8_t i = 0; i < numOfSensor; i++) 
  {

    long value = map(_sensorValues[i], _min_sensor_values[i], _max_sensor_values[i], 100, 0);
    if (colorLine == 1) { value = 100 - value; }                                                                            
    if (value > 25) {
      onLine = true;
    }
  ...
  if (!onLine)
  {
    if (_lastPosition < (numOfSensor - 1) * 100 / 2)
    {
      return  0 - ((numOfSensor - 1) * 100)/2;
    }
    else 
    {
      return ((numOfSensor - 1) * 100) - ((numOfSensor - 1) * 100)/2;
    }
  }
  _lastPosition = avg / sum;
  return _lastPosition - ((numOfSensor - 1) * 100)/2; 

}

/*void QTRSensors::set_minimum_sensor(uint8_t addressPin, uint16_t sensor_Values){
  calibrationOn.minimum[addressPin] = sensor_Values;
}
void QTRSensors::set_maximum_sensor(uint8_t addressPin, uint16_t sensor_Values){
  calibrationOn.maximum[addressPin] = sensor_Values;
}*/
